...
import { ref, push, onChildAdded, get, update, onValue, remove, query, orderByChild, startAt } from "https://www.gstatic.com/firebasejs/12.9.0/firebase-database.js";
import {
  initCall,
...
function listenForNewMessages() {
  // Only listen for messages arriving from NOW — history is handled by loadMessagesFromLocal + syncFromFirebase
  const listenFromTime = Date.now();
  const messagesRef = query(
    ref(db, 'messages/' + chatKey),
    orderByChild('timestamp'),
    startAt(listenFromTime)
  );
